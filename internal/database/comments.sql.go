// Code generated by sqlc. DO NOT EDIT.
// versions:
//   sqlc v1.27.0
// source: comments.sql

package database

import (
	"context"
	"database/sql"

	"github.com/google/uuid"
)

const createComment = `-- name: CreateComment :one
INSERT INTO comments (id, file_path, commit_hash, line_start, line_end, char_start, char_end, author, body, resolved)
VALUES (
    gen_random_uuid(),
    $1,
    $2,
    $3,
    $4,
    $5,
    $6,
    $7,
    $8,
    $9
)
<<<<<<< HEAD
RETURNING id, file_path, repo, commit_hash, line_start, line_end, author, body, created_at, updated_at, resolved, user_id
=======
RETURNING id, file_path, repo, commit_hash, line_start, line_end, author, body, created_at, updated_at, resolved, char_start, char_end
>>>>>>> 6d1cce4 (work so far in the revision comment program)
`

type CreateCommentParams struct {
	FilePath   string
	CommitHash string
	LineStart  int32
	LineEnd    int32
	CharStart  int32
	CharEnd    int32
	Author     string
	Body       string
	Resolved   sql.NullBool
}

func (q *Queries) CreateComment(ctx context.Context, arg CreateCommentParams) (Comment, error) {
	row := q.db.QueryRowContext(ctx, createComment,
		arg.FilePath,
		arg.CommitHash,
		arg.LineStart,
		arg.LineEnd,
		arg.CharStart,
		arg.CharEnd,
		arg.Author,
		arg.Body,
		arg.Resolved,
	)
	var i Comment
	err := row.Scan(
		&i.ID,
		&i.FilePath,
		&i.Repo,
		&i.CommitHash,
		&i.LineStart,
		&i.LineEnd,
		&i.Author,
		&i.Body,
		&i.CreatedAt,
		&i.UpdatedAt,
		&i.Resolved,
<<<<<<< HEAD
		&i.UserID,
=======
		&i.CharStart,
		&i.CharEnd,
>>>>>>> 6d1cce4 (work so far in the revision comment program)
	)
	return i, err
}

const getComment = `-- name: GetComment :one
<<<<<<< HEAD
SELECT id, file_path, repo, commit_hash, line_start, line_end, author, body, created_at, updated_at, resolved, user_id FROM comments WHERE id=$1
=======
SELECT id, file_path, repo, commit_hash, line_start, line_end, author, body, created_at, updated_at, resolved, char_start, char_end FROM comments WHERE id=$1
>>>>>>> 6d1cce4 (work so far in the revision comment program)
`

func (q *Queries) GetComment(ctx context.Context, id uuid.UUID) (Comment, error) {
	row := q.db.QueryRowContext(ctx, getComment, id)
	var i Comment
	err := row.Scan(
		&i.ID,
		&i.FilePath,
		&i.Repo,
		&i.CommitHash,
		&i.LineStart,
		&i.LineEnd,
		&i.Author,
		&i.Body,
		&i.CreatedAt,
		&i.UpdatedAt,
		&i.Resolved,
<<<<<<< HEAD
		&i.UserID,
=======
		&i.CharStart,
		&i.CharEnd,
>>>>>>> 6d1cce4 (work so far in the revision comment program)
	)
	return i, err
}

const getCommentsByUser = `-- name: GetCommentsByUser :many
SELECT id, file_path, repo, commit_hash, line_start, line_end, author, body, created_at, updated_at, resolved, user_id FROM comments WHERE user_id=$1
`

func (q *Queries) GetCommentsByUser(ctx context.Context, userID uuid.UUID) ([]Comment, error) {
	rows, err := q.db.QueryContext(ctx, getCommentsByUser, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []Comment
	for rows.Next() {
		var i Comment
		if err := rows.Scan(
			&i.ID,
			&i.FilePath,
			&i.Repo,
			&i.CommitHash,
			&i.LineStart,
			&i.LineEnd,
			&i.Author,
			&i.Body,
			&i.CreatedAt,
			&i.UpdatedAt,
			&i.Resolved,
			&i.UserID,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Close(); err != nil {
		return nil, err
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}
